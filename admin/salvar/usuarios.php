<?php
    ...
    if($_POST){
        ...
        if ( empty ($senha ) ) {
            ...
            $sql = "update funcionario set  login = :login, modificado = :modificado  where id = :id limit 1";
            $consulta = $pdo->prepare($sql);
            $consulta->bindParam(":login", $login);
            ...
            $sql = "update funcionario set login = :login, senha = :senha, modificado = :modificado where id = :id limit 1";
            $consulta = $pdo->prepare($sql);
            $consulta->bindParam(":id", $idfuncionario);
            $consulta->bindParam(":login", $login);
